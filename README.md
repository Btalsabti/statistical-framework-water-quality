[![DOI](https://zenodo.org/badge/1160528665.svg)](https://doi.org/10.5281/zenodo.19327592)

# Reproducible Statistical Inference for Assessing Water Quality: A Decision Framework for Method Selection and Reporting
## Overview
An **interactive statistical decision-tree framework** has been developed to guide the selection of appropriate statistical methods for reproducible water quality assessment and hydrochemical analysis. It is designed to assist researchers, students, and environmental scientists in navigating statistical analysis workflows commonly used in water and environmental studies.

The framework integrates commonly used **univariate, bivariate, and multivariate statistical techniques** and provides structured guidance on when and how to apply each method. The tool was developed using **Python and Dash** to create an interactive web-based decision-support platform.


---

## Interactive Web Application

The interactive decision-tree framework is available online:

https://statistical-framework-water-quality.onrender.com

The web application allows users to:

- Explore the statistical decision tree interactively  
- Select statistical methods using the **Quick Method Selector**
- Step-wise statistical method selection starting from sample size and normality checks.
- View descriptions, interpretations, limitations, and references for each method  
- Export visual representations of the statistical framework  
- Switch between **light and dark visualization modes**

---
## Repository Contents

- `framework_app.py` – Main Dash application  
- `requirements.txt` – Python dependencies  
- `assets/` – Images used in the interface  
- `README.md` – Project documentation

---

## Publication

The interactive statistical framework was developed as part of a research study on statistical approaches for water quality assessment.

The interactive tool is provided as **Supplementary Material (S3)** accompanying the manuscript.

---

## Code Availability

The complete source code of the interactive framework is publicly available at:

https://github.com/Btalsabti/statistical-framework-water-quality.git

---

## Authors

Bedour Alsabti¹; Todd Robinson¹; Chidambaram Sabarathinam²; Prasanna Mohan Viswanathan³; Domenik Wolff-Boenisch¹  
¹ School of Earth and Planetary Sciences, Curtin University, Perth, Australia  
² Water Research Center, Kuwait Institute for Scientific Research, Kuwait  
³ Faculty of Engineering and Science, Curtin University, Miri, Malaysia  

---

## License

This project is intended for **academic and research use**.  
Kindly cite the associated publication when using this framework in research.

